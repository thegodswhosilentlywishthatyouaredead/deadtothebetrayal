...
def calculate_ticket_aging(tickets, now):
    ...
    if active_tickets > 0:
        aging_data["averageAge"] = round(total_age_minutes / active_tickets, 1)
        aging_data["oldestTicket"] = round(oldest_age_minutes, 1)
...
def calculate_team_productivity():
    ...
    if len(field_teams) > 0:
        productivity_data["averageCompletionTime"] = round(total_completion_time / len(field_teams), 1)
        ...
        productivity_data["productivityScore"] = round((avg_rating * 20) + (avg_tickets * 0.5), 1)
...
def calculate_team_efficiency_score(team):
    ...
    return round((tickets_completed * 0.3) + (rating * 20), 1)
    ...
    try:
        ...
        for zone in zones.values():
            if zone["totalTickets"] > 0:
                ...
                zone["productivityScore"] = round(((zone["closedTickets"] - zone["openTickets"]) / zone["totalTickets"]) * 100, 1)
            ...
            if zone["totalTeams"] > 0:
                ...
                zone["averageRating"] = round(total_rating / zone["totalTeams"], 1)
                zone["averageEfficiency"] = round(total_efficiency / zone["totalTeams"], 1)
            
            for state in zone["states"].values():
                if state["totalTickets"] > 0:
                    state["productivityScore"] = round(((state["closedTickets"] - state["openTickets"]) / state["totalTickets"]) * 100, 1)
                ...
                if len(state["teams"]) > 0:
                    ...
                    state["averageRating"] = round(state_rating / len(state["teams"]), 1)
                    state["averageEfficiency"] = round(state_efficiency / len(state["teams"]), 1)
...
if __name__ == '__main__':
    print("🚀 Starting FieldAssign Backend Server...")
